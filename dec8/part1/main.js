  ...
  for (let i = antennaPosition.y; i < antennaMatrix.length; i++) {
    for (let j = 0; j < antennaMatrix.length; j++) {
      ...
      if (isSameAntenna) {
        ...
        const possibleAntinodes2 = lengthBetweenNodes
          .get(length)
          .get(getKey({ y: i, x: j }));

        for (let z = 0; z < possibleAntinodes2.length; z++) {
          const key = possibleAntinodes2[z];
          const antinode = key.split(":");
          const r = getAngleRad(
            { y: i, x: j },
            { y: antinode[0], x: antinode[1] }
